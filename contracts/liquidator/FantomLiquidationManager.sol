    ...
    ReentrancyGuard
{
        ...
        nonReentrant
    {
        ...
        for (index = 0; index < _auction.debtList.length; index++) {
            ...
            _auction.debtValue[index] = _auction.debtValue[index].sub(
                debtAmount
            );
        }
        ...
        onlyNotContract
    {
        ...
        for (index = 0; index < tokenCount; index++) {
            ...
            if (
                FantomMintTokenRegistry(
                    addressProvider.getAddress(MOD_TOKEN_REGISTRY)
                ).canTrade(tokenAddress)
            ) {
        ...
        for (index = 0; index < tokenCount; index++) {
            ...
            if (tokenBalance > 0) {
                ...
                _auction.debtValue.push(
                    tokenBalance.mul(STABILITY_RATIO).div(100)
                );
            }
        }
